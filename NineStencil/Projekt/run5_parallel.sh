#! /bin/bash


gcc -O3 -fopenmp -o  gridP ./main.c -lm
echo
echo "Parallel 5-point stencil v-cycle"
echo
echo "First start with Jacobi and 255x255 grid"
echo
./gridP 255 4 jac 5 v
echo
echo "Now the same 255x255 grid with Gauss-Seidel"
echo
./gridP 255 4 gau 5 v
echo
echo "And now a 255x255 grid with SOR"
echo
./gridP 255 4 sor 5 v
echo
echo "Use the best smoothing operator SOR for a 511x511 grid now"
echo
./gridP 511 5 sor 5 v
echo
echo "And now SOR for a 751x751 grid"
echo
./gridP 751 5 sor 5 v


echo
echo "We now use the 5-point stencil with the w-cycle"
echo
echo "Again first with Jacobi and a 255x255 grid"
echo
./gridP 255 4 jac 5 w
echo
echo "Now the same 255x255 grid with Gauss-Seidel"
echo
./gridP 255 4 gau 5 w
echo
echo "And now a 255x255 grid with SOR"
echo
./gridP 255 4 sor 5 w
echo
echo "Again a 511x511 grid with SOR"
echo
./gridP 511 5 sor 5 w
echo
echo "And SOR for a 751x751 grid"
echo
./gridP 751 5 sor 5 w


echo
echo "Now at the end of the 5-point stencil we also try the mu3-cycle"
echo
echo "Jacobi: 255x255 grid"
echo
./gridP 255 4 jac 5 mu 3
echo
echo "Gauss-Seidel: 255x255 grid"
echo
./gridP 255 4 gau 5 mu 3
echo
echo "sor: 255x255 grid"
echo
./gridP 255 4 sor 5 mu 3
echo
echo "sor: 511x511 grid"
echo
./gridP 511 5 sor 5 mu 3
echo
echo "sor: 751x751 grid"
echo
./gridP 751 5 sor 5 mu 3
echo
echo "End"
echo



exit 0
