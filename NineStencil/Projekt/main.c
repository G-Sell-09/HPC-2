 ...
 *
 * @param[out] {0,1}   returns 0 if the method finished wiN_pot[L-1thout errors, 1 otherwise
 *
 * \author Robin Sell
 * \author Neil Vetter
 *
...
int main (int argc , char **argv)
{
  ...
  // calc. gridsizes (and squared) for every level
  ...
  for (i=1;i<=maxIter;i++)
  {
    ...
    // call to the multigrid method
    cycle(ArrOfx, ArrOfb, ArrOfr, L, L, nu1, nu2, mu, w, tolSmoo, N_vec, N_pot, A, smooth, mfMult);
    // calculate residual norm
    e_iter[i] = norm2(ArrOfr[0],N*N);
    // breaking condition for the residual norm
    if (e_iter[i]<tol)
      break;
