 ...
 *
 * @param[in] x   	vector
 * @param[in] ax  	vector for the matvec product
 * @param[in] N  		gridsize
 *
 * @param[out] ax		calculated matrix-vector product
 *
 * \author Robin Sell
 * \author Neil Vetter
 *
...
void mfMult9(double* x, double* ax, int N)
{
	...
	#pragma omp parllel
	{
		...
		for ( int i = 1; i < N-1; i++ )
		{
			ax[i]         = 20*x[i]         - 4*x[i-1]         - 4*x[i+1]           - 4*x[i+N]        - x[i+N-1]       - x[i+N+1]   ;    // bottom edge
			ax[(N-1)*N+i] = 20*x[(N-1)*N+i] - 4*x[(N-1)*N+i-1] - 4*x[(N-1)*N+i+1]   - 4*x[(N-2)*N+i] - x[(N-2)*N+i-1] - x[(N-2)*N+i+1]; // top edge
			ax[i*N]       = 20*x[i*N]       - 4*x[i*N + 1]     - 4*x[(i-1)*N]       - 4*x[(i+1)*N]    - x[(i-1)*N+1]   - x[(i+1)*N+1]  ; // left edge
			ax[i*N+N-1]   = 20*x[i*N+N-1]   - 4*x[i*N+N-2]     - 4*x[(i-1)*N+N-1] - 4*x[(i+1)*N+N-1] - x[(i-1)*N+N-2] - x[(i+1)*N+N-2];  // right edge
		}

		#pragma omp single
		{
		// corners
		ax[0]             = 20*x[0]           - 4*x[1]           - 4*x[N]           - x[N+1];  		// bottom left
		ax[N-1]           = 20*x[N-1]         - 4*x[N-2]         - 4*x[2*N-1]       - x[2*N-2];		// bottom right
		ax[(N-1)*N]       = 20*x[(N-1)*N]     - 4*x[(N-1)*N+1]   - 4*x[(N-2)*N]     - x[(N-2)*N+1] ;		// top left
		ax[(N-1)*N + N-1] = 20*x[(N-1)*N+N-1] - 4*x[(N-1)*N+N-2] - 4*x[(N-2)*N+N-1] - x[(N-2)*N+N-2];	// top right
		}
	}
}
